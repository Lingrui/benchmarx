...
class Challenge2018Score:
    ...
    def gross (self):
        return self._auc(self._pos_values, self._neg_values)
...
if __name__ == '__main__':
    ...
    DATA_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '../data/physionet18'))
    ...
    failed = 0
    for vec_file in glob(result_dir + '/*.vec'):
        record = os.path.basename(vec_file)
        if record.endswith('.vec'):
            record = record[:-4]

        if record[1] == 'r':
            reference_dir = os.path.join(DATA_ROOT, 'training')
        elif record[1] == 'e':
            reference_dir = os.path.join(DATA_ROOT, 'test')
        else:
            assert False

        arousal_file = os.path.join(reference_dir, record,
                                    record + '-arousal.mat')
        try:
